...
;(function($) {
	...
	$.datePicker = {
		...
		strings: {
			...
			messageLocked: 'День, который вы только что выбрали, недоступен'
		},
		defaults: {
		...
		show: function(options) {
			...
			var calendar = $.datePicker.utils.buildCalendar(selected.year, selected.month, date),
				...
				datePickerC = $('<div class="datepicker"><span class="tip"></span></div>'),
				datePicker = $('<div class="dp-cover">').append(datePickerC);

			// Create the markup elements
			datePickerC.append(calendar);
			datePickerC.append(months);
			datePickerC.append(years);
			datePickerC.append(decades);
			...
			if (opts.element && ! opts.appendTo) {
				...
				var w = $(window).width();
				var elw = datePicker.width();
				datePicker.css({
					left: w <= 320 ? elw > w ? 0 : (w - elw) / 2 : offset.left + 'px',
					top: offset.top + opts.element.outerHeight(true) + 15 + 'px'
				});
				if (w <= 320) {
					var l = 0;
					$(opts.element).prevAll().each(function () {
						l += this.scrollWidth;
                    });
					var padd = ($(opts.element).innerWidth() - $(opts.element).width()) / 2;
					$('.tip').css('left', l - padd + 15 + 'px');
                }
			}
			...
			datePicker.on('click', '.calendar .day', function(e) {
				...
				if ( opts.selectDate( test ) ) {
					...
					var auto_form = $(opts.element).closest('form[data-autosubmit]');
                    if (auto_form.length) {
                      auto_form.submit();
                    }
				}
			...
			datePicker.on('click', '.calendar .month', function(e) {
				...
				var el = $(this),
					calendar = el.closest('.calendar'),
					months = datePickerC.children('.months'),
					picker = $.datePicker.utils.buildMonthPicker(selected.year, selected.month);
				console.log('months',months,datePicker);
				// Rebuild picker
				months.replaceWith(picker);
				months = picker;
				// Animate
				console.log('months.fadeIn0',months.css('display'));
				calendar.fadeOut(150, function() {
					months.fadeIn(150);
					$('.months').fadeIn(150);
					console.log('months.fadeIn1',months.css('display'));
				});
				console.log('months.fadeIn2',months.css('display'));
			});
			...
			datePicker.on('click', '.months .month', function(e) {
				...
				var el = $(this),
					...
					month = el.data('month'),
					year = el.data('year'),
					calendar = datePickerC.children('.calendar'),
					replacement = null;
				if ( el.hasClass('blank') ) {
					return;
				}
			...
			datePicker.on('click', '.years .decade', function(e) {
				...
				var el = $(this),
					years = el.closest('.years'),
					decades = datePickerC.children('.decades');
				// Animate
				years.fadeOut(150, function() {
					decades.fadeIn(150);
				});
	...
	$(function() {
		...
		/**** >> add custom ****/
		$('.dp-left,.dp-right,.dp-today').on('click',function() {
			var el = $('.dp-group input');
			var pd = new Date();
			if (!$(this).is('.dp-today')) {
				var d = el.val();
				var pd = $.datePicker.defaults.parseDate(d);
				if (d) {
					var dl = $(this).is('.dp-left') ? -1 : 1;
					pd.setDate(pd.getDate() + dl);
				}
			}
			el.val($.datePicker.defaults.formatDate(pd));
			el.trigger('change');
			var auto_form = $(this).closest('form[data-autosubmit]');
            if (auto_form.length) {
                auto_form.submit();
            }
		});
